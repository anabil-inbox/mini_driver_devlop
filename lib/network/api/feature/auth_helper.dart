...
class AuthHelper {
  ...
  Future<AppResponse> checkVerficationCode(Map<String, dynamic> body) async {
    var appResponse = await AuthApi.getInstance.checkVerficationCodeRequset(
        body: body,
        url: ConstanceNetwork.verficationEndPoint,
        header: ConstanceNetwork.header(0));
    if (appResponse.status?.success == true) {
      return appResponse;
    } else {
  ...
  Future<AppResponse> reSendVerficationCode(Map<String, dynamic> body) async {
    var appResponse = await AuthApi.getInstance.reSendVerficationCodeRequset(
        body: body,
        url: ConstanceNetwork.recendVerficationCodeEndPoint,
        header: ConstanceNetwork.header(0));
    if (appResponse.status?.success == true) {
      return appResponse;
    } else {
